...
Data = readtable(strcat(Filename,'_obs_fusion.csv'));
Fusion_Info.signals.dimensions=79;
Fusion_Info.signals.values = zeros(79,1);
j=0;
m=1;
for i=2:1:length(Data.local_id)+1
    if i<=length(Data.local_id)
        if (Data.local_id(i-1)==Data.local_id(i))
            if (j<=9)
                Fusion_Info.signals.values(j*7+3,m) = Data.obs_rel_x(i-1);
                Fusion_Info.signals.values(j*7+4,m) = -Data.obs_rel_y(i-1);
                Fusion_Info.signals.values(j*7+5,m) = Data.obs_x(i-1);
                Fusion_Info.signals.values(j*7+6,m) = -Data.obs_y(i-1);
                Fusion_Info.signals.values(j*7+7,m) = Data.vel_x(i-1);
                Fusion_Info.signals.values(j*7+8,m) = -Data.vel_y(i-1);
                Fusion_Info.signals.values(j*7+9,m) = Data.id(i-1);
                j=j+1;
            else
                j=9;
                Fusion_Info.signals.values(j*7+3,m) = Data.obs_rel_x(i-1);
            ...
            Fusion_Info.signals.values(2,m) = Data.local_id(i-1);
            Fusion_Info.time(m,1) = Data.local_id(i-1)*0.02;
            j=0;
            m=m+1;
        end
    else
